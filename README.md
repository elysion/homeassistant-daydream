...
http {
...
  # This map is needed for development because the template uses Web Sockets for live reload.
  # If you don't want to use the development mode (npm start) but instead just want to serve the static page
  # (e.g. using: npm run build && pushstate-server build) you can leave out this section.
  ...
  server {
    ...
    location / {
      proxy_pass http://0.0.0.0:3000; # The frontend server uses port 3000 by default. You can override it using the PORT environment variable

      proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
      proxy_set_header Host $host;  # pass the host header - http://wiki.nginx.$

      proxy_http_version 1.1;  # recommended with keepalive connections - http:$ # This is probably only needed for development setup
    ...
    location /api {
      rewrite ^ $request_uri?api_password=home-assistant_api_password break; # Insert your Home Assistant API password here
      proxy_pass http://0.0.0.0:8123; # Update the port here, if you have defined a custom port in Home Assistant http component configuration
      proxy_redirect off;
    }
  }
  ...
